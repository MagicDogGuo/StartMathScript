...
public class MoveItemControl_PermutationsSum : MonoBehaviour {
    [HideInInspector]
    ...
    private void OnMouseDown()
    {
        if (this.CanLeaveClone == true)
        {
            ...
            go.GetComponent<MoveItemControl_PermutationsSum>().CanLeaveClone = true;
    ...
    private void OnMouseUp()
    {
        ...
        else if (m_ColliderObj != null && m_ColliderObj.tag == "removeItem")
        {
            Destroy(this.gameObject);
        }
    ...
    #region OnTrigger
    private GameObject tmp_matchItem;
    
    private void OnTriggerStay2D(Collider2D collision)
    {

        //判斷數量跟tag  
        if (collision.tag == "removeItem")
        {
            m_ColliderObj = collision.gameObject;
        }
        else if (collision.tag == "matchPosItem")
        {
            //if (collision.GetComponent<MatchPosItemControl_matchC>().OnCollidionrObjCount() == 0)
            //{
            //    m_ColliderObj = collision.gameObject;
            //}
            tmp_matchItem = collision.gameObject;
            m_ColliderObj = collision.gameObject;
        }
        else if (collision.tag == "moveItem")
        {
            if (tmp_matchItem != null)
                m_ColliderObj = tmp_matchItem;
            else
